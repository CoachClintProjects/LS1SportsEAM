...
// Keep server-side user-token validation on the same public key preference as the browser client.
// A stale legacy ANON key must never override the current publishable key and make a freshly
// issued browser session appear invalid to the server.
...
export async function verifySuperUserToken(token: string | null | undefined): Promise<SuperUserOperator | null> {
  ...
  const operatorResponse = await fetch(
    `${SUPABASE_URL}/rest/v1/platform_superuser_operators?select=email,display_name,can_onboard_clients,can_manage_platform_settings&active=eq.true&email=eq.${encodeURIComponent(email)}&limit=1`,
    {
...
export async function getAuthorizedSuperUser(request: NextRequest): Promise<SuperUserOperator | null> {
  return verifySuperUserToken(request.cookies.get('ls1_superuser_session')?.value);
}
