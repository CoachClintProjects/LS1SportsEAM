...
export async function PATCH(request:NextRequest){
 ...
 const {url}=supabaseServerConfig(); const headers=serviceHeaders(); if(!headers)return NextResponse.json({error:'Server authorization is not configured.'},{status:500});
 const response=await fetch(`${url}/rest/v1/rpc/admin_replace_role_assignments`,{method:'POST',headers:{...headers,Prefer:'return=representation'},body:JSON.stringify({p_actor_user_id:ctx.user.id,p_actor_person_id:ctx.person.id,p_target_person_id:personId,p_assignments:requested.map((raw:any)=>({roleDefinitionId:clean(raw?.roleDefinitionId),organizationId:clean(raw?.organizationId),sportId:clean(raw?.sportId),siteId:clean(raw?.siteId),teamId:clean(raw?.teamId),programId:clean(raw?.programId),competitionId:clean(raw?.competitionId),startsAt:clean(raw?.startsAt),endsAt:clean(raw?.endsAt)})),p_reason:clean(body?.reason)||'Client access administration'}),cache:'no-store'});
 const json=await response.json().catch(()=>({})); if(!response.ok)return NextResponse.json({error:'Role assignment was rejected by the authorization boundary.',detail:json?.message||null},{status:403,headers:noStore});
