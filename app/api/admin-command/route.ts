...
export async function POST(request:NextRequest){
 try{const ctx=await resolveAccess(request);if(!ctx)return deny(401,'Authentication required.');if(!ctx.allowedHubs.includes('admin'))return deny(403,'Admin access denied.');
  ...
  if(action==='create-event'){if(roleName!=='org_admin'||!roleHas(ctx,roleName,'record.update'))return deny(403,'Event creation denied.');const name=String(body.name||'').trim(),startsAt=String(body.startsAt||''),endsAt=body.endsAt?String(body.endsAt):null;if(!name||!startsAt||Number.isNaN(Date.parse(startsAt)))return deny(400,'Event name and valid start time are required.');const org=organizationIds(ctx)[0];if(!org)return deny(403,'Organization scope required.');const row=await rest('competitions',{method:'POST',body:JSON.stringify({organization_id:org,name,competition_type:'event',starts_at:startsAt,ends_at:endsAt,timezone:'America/Edmonton',status:'scheduled'})});const created=row?.[0];if(!created)throw new Error('Event creation returned no record.');await audit(ctx,tenant,'event.created','competition',created.id,null,created,correlationId);return NextResponse.json({ok:true,row:created,correlationId});}
