...
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY ?? process.env.SUPABASE_SECRET_KEY ?? process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
...
export async function GET() {
  try {
    const projects = await read('platform_projects', 'select=id,name,status,start_date,target_date,description,code&code=eq.LS1SPORTS-IMPLEMENTATION&limit=1');
    ...
    const countTables = ['organizations','people','athletes','teams','team_memberships','programs','seasons','competitions','competition_results','import_jobs','data_quality_issues','approvals','audit_events','ai_agents','automation_definitions','integration_connections','assets'];
    const entries = await Promise.all(countTables.map(async table => [table, await count(table)] as const));

    return NextResponse.json({ project, milestones, tasks, raci, counts: Object.fromEntries(entries), generatedAt: new Date().toISOString(), source: 'LS1SportsEAM Supabase' }, { headers: { 'Cache-Control': 'no-store, max-age=0' } });
  } catch (error) {
    return NextResponse.json({ project: null, milestones: [], tasks: [], raci: [], counts: {}, generatedAt: new Date().toISOString(), source: 'LS1SportsEAM Supabase', error: error instanceof Error ? error.message : 'Command data unavailable' }, { status: 200, headers: { 'Cache-Control': 'no-store' } });
  }
}
