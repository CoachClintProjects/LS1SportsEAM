...
const REFS:Record<string,RefConfig>={
 ...
 programs:{label:'Programs',select:'id,code,name,program_type,status,organization_id',search:['code','name','program_type','status'],display:['name','code','program_type','status']},
 seasons:{label:'Seasons',select:'id,code,name,starts_on,ends_on,status,organization_id',search:['code','name','status'],display:['name','code','status']},
 teams:{label:'Teams',select:'id,code,name,competitive_level,status,organization_id,program_id,season_id',search:['code','name','competitive_level','status'],display:['name','code','competitive_level','status']},
 team_memberships:{label:'Roster assignments',select:'id,team_id,athlete_id,person_id,membership_type,status,starts_on,ends_on',search:['membership_type','status'],display:['membership_type','status','team_id']},
 memberships:{label:'Memberships',select:'id,organization_id,person_id,membership_number,membership_type,status,starts_on,ends_on',search:['membership_number','membership_type','status'],display:['membership_number','membership_type','status']},
...
function labelFor(row:Record<string,unknown>,config:RefConfig){const values=config.display.map(key=>row[key]).filter(value=>value!==null&&value!==undefined&&String(value).trim()!=='').map(String);return values.join(' · ')||String(row.id||'Record')}
export async function GET(request:NextRequest){try{await requireSuperUser(request);if(!URL||!KEY)throw new Error('Supabase service credentials are not configured.');const table=String(request.nextUrl.searchParams.get('table')||'').trim(),config=REFS[table];if(!config)return NextResponse.json({error:'Reference type is not exposed in the active Team Engine scope.'},{status:403});const query=safeTerm(String(request.nextUrl.searchParams.get('q')||''));let path=`${table}?select=${encodeURIComponent(config.select)}&limit=25`;if(query)path+=`&or=(${config.search.map(field=>`${field}.ilike.*${encodeURIComponent(query)}*`).join(',')})`;const response=await fetch(`${URL}/rest/v1/${path}`,{headers:{apikey:KEY,Authorization:`Bearer ${KEY}`},cache:'no-store'}),text=await response.text();if(!response.ok)throw new Error(`Reference lookup failed: ${response.status} ${text.slice(0,300)}`);const rows=text?JSON.parse(text):[];return NextResponse.json({table,label:config.label,options:(rows||[]).map((row:Record<string,unknown>)=>({id:row.id,label:labelFor(row,config),data:row}))},{headers:{'Cache-Control':'no-store'}})}catch(error){if(error instanceof SuperUserAuthError)return NextResponse.json({error:error.message},{status:error.status});return NextResponse.json({error:error instanceof Error?error.message:'Reference lookup failed.'},{status:500})}}
