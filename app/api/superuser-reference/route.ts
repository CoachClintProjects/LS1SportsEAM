...
export async function GET(request:NextRequest){try{const actor=await requireSuperUser(request);const table=String(request.nextUrl.searchParams.get('table')||'').trim(),config=REFS[table];if(!config)return NextResponse.json({error:'Reference type is not exposed in the active Team Engine scope.'},{status:403});const query=safeTerm(String(request.nextUrl.searchParams.get('q')||''));let path=`${table}?select=${encodeURIComponent(config.select)}&limit=25`;if(query)path+=`&or=(${config.search.map(field=>`${field}.ilike.*${encodeURIComponent(query)}*`).join(',')})`;const rows=await superUserRest<any[]>(actor,path);return NextResponse.json({table,label:config.label,options:(rows||[]).map((row:Record<string,unknown>)=>({id:row.id,label:labelFor(row,config),data:row}))},{headers:{'Cache-Control':'no-store'}})}catch(error){if(error instanceof SuperUserAuthError)return NextResponse.json({error:error.message},{status:error.status});return NextResponse.json({error:error instanceof Error?error.message:'Reference lookup failed.'},{status:500})}}
