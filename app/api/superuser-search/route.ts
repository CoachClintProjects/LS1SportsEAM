...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = (path: string) => superUserRest<any[]>(actor, path);
    ...
    const results: SearchResult[] = [
      ...people.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Person', title: [item.preferred_name || item.first_name, item.last_name].filter(Boolean).join(' ') || String(item.email || 'Person'), subtitle: [item.email, item.status].filter(Boolean).join(' · '), href: '/superuser?view=people' })),
      ...organizations.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Organization', title: String(item.name || item.legal_name || item.code || 'Organization'), subtitle: [item.code, item.status].filter(Boolean).join(' · '), href: '/superuser?view=organizations' })),
      ...teams.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Team', title: String(item.name || item.code || 'Team'), subtitle: [item.code, item.competitive_level, item.status].filter(Boolean).join(' · '), href: '/superuser?view=team-engine' })),
      ...tasks.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Project task', title: `${String(item.code || 'TASK')} · ${String(item.name || 'Unnamed')}`, subtitle: `${String(item.status || 'unknown')} · ${Number(item.percent_complete || 0)}%`, href: '/superuser?view=command-center' })),
      ...onboarding.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Onboarding', title: String(item.client_name || 'Client onboarding'), subtitle: `${String(item.status || 'unknown')} · step ${Number(item.current_step || 0)}/12`, href: `/superuser/onboarding?case=${encodeURIComponent(String(item.id))}` })),
      ...knowledge.map((item: Record<string, unknown>) => ({ id: String(item.id), type: 'Knowledge', title: String(item.title || 'Knowledge item'), subtitle: [item.category, item.status].filter(Boolean).join(' · '), href: '/superuser?view=knowledge' })),
    ].slice(0, 30);
