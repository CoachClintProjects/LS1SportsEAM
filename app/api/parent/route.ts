...
export async function GET(request:NextRequest){
 try{
  ...
  if(!familyId)return NextResponse.json({family:null,members:[],athletes:[],tasks:[],messages:[],invoices:[],documents:[],trips:[],custody:[],volunteer:[],memberships:[],metrics:{},source:'LS1SportsEAM Supabase',setupRequired:true});
  const [hub,memberships]=await Promise.all([
  ...
  return NextResponse.json({...hub,memberships:memberships||[],source:'LS1SportsEAM Supabase'},{headers:{'Cache-Control':'no-store, max-age=0'}});
...
export async function POST(request:NextRequest){
 try{
  ...
  if(action==='submit-membership-request'){
   const result=await parentRest<Row>(actor,'rpc/parent_submit_membership_request',{method:'POST',body:JSON.stringify({
    p_family_id:familyId,
    p_membership_id:String(b.membership_id||''),
    p_request_kind:String(b.request_kind||''),
    p_parent_summary:String(b.parent_summary||''),
    p_amount:b.amount===''||b.amount==null?null:Number(b.amount),
    p_requested_start_on:b.requested_start_on||null,
    p_requested_end_on:b.requested_end_on||null,
    p_acknowledged:Boolean(b.acknowledged),
    p_acknowledgement_text:String(b.acknowledgement_text||''),
   })});
