...
type ScopeUnitRow={unit_id:string;required:boolean;reason:string|null};
...
export async function GET(request:NextRequest){try{await requireSuperUser(request);const scopes=await rest<ScopeRow[]>('platform_delivery_scopes?scope_key=eq.superuser-team-engine&status=eq.active&select=id,scope_key,name,description,status&limit=1');const scope=scopes[0]||null;if(!scope)return NextResponse.json({error:'Active Super User / Team Engine delivery scope is not configured.'},{status:404});const[milestones,units]=await Promise.all([rest<ScopeMilestoneRow[]>(`platform_delivery_scope_milestones?scope_id=eq.${encodeURIComponent(scope.id)}&select=milestone_id,required,display_mode,sort_order&order=sort_order.asc`),rest<ScopeUnitRow[]>(`platform_delivery_scope_units?scope_id=eq.${encodeURIComponent(scope.id)}&select=unit_id,required,reason`)]);return NextResponse.json({scope,milestones,units,requiredMilestoneIds:milestones.filter(item=>item.required).map(item=>item.milestone_id),roadmapMilestoneIds:milestones.filter(item=>!item.required).map(item=>item.milestone_id),excludedUnitIds:units.filter(item=>!item.required).map(item=>item.unit_id),unitOverrides:units,generatedAt:new Date().toISOString(),source:'LS1SportsEAM delivery scope'},{headers:{'Cache-Control':'no-store, max-age=0, must-revalidate'}})}catch(error){if(error instanceof SuperUserAuthError)return NextResponse.json({error:error.message},{status:error.status});return NextResponse.json({error:error instanceof Error?error.message:'Delivery scope unavailable.'},{status:500})}}
