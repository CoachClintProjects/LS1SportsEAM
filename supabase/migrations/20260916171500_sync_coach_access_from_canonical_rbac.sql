...
begin
 select code into v_code from public.role_definitions where id=coalesce(new.role_definition_id,old.role_definition_id);
 if v_code <> 'COACH' then return coalesce(new,old); end if;
 if tg_op='DELETE' then update public.coach_access_assignments set status='inactive',updated_at=now() where coach_person_id=old.person_id and team_id is not distinct from old.team_id and program_id is not distinct from old.program_id; return old; end if;
 v_org:=new.organization_id; if v_org is null and new.team_id is not null then select organization_id into v_org from public.teams where id=new.team_id; end if;
 ...
 do update set tenant_id=excluded.tenant_id,status=excluded.status,starts_on=excluded.starts_on,ends_on=excluded.ends_on,is_head_coach=excluded.is_head_coach,updated_at=now(); return new;
end$$;
