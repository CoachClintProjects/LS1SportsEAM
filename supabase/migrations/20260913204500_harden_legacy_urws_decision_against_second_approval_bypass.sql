create or replace function public.urws_record_authorized_decision(p_case_id uuid, p_human_outcome text, p_rationale text, p_financial_impact numeric default null::numeric, p_currency character default 'CAD'::bpchar)
returns uuid
language plpgsql
security definer
set search_path to 'public','app'
as $function$
declare c public.urws_cases%rowtype; v_person uuid; v_operator uuid; v_auth uuid; v_id uuid; v_evidence jsonb; v_previous uuid; v_tenant uuid; v_rule uuid; v_requires_second boolean:=false;
begin
 ...
 if v_rule is not null then select requires_second_approval into v_requires_second from public.urws_authority_rules where id=v_rule; end if;
 if coalesce(v_requires_second,false) then raise exception 'This URWS decision requires a governed proposal and second approval; direct decision recording is prohibited' using errcode='42501'; end if;
