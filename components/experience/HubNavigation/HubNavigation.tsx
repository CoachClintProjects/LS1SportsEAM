...
const superuserExtensions: NavigationSection[] = [
  {
    ...
    items: [
      { id: 'all-clients', label: 'All Clients', icon: 'building' },
      { id: 'new-client', label: 'New Client', icon: 'user-plus' },
      { id: 'onboarding-queue', label: 'Onboarding Queue', icon: 'workflow' },
      { id: 'active-clients', label: 'Active Clients', icon: 'check' },
      { id: 'client-exceptions', label: 'At-Risk / Exceptions', icon: 'alert' },
      { id: 'client-updates', label: 'Existing Client Updates', icon: 'upload' },
    ],
  },
  {
    id: 'configuration',
    ...
    items: [
      { id: 'settings', label: 'Platform / Site Settings', icon: 'settings' },
      { id: 'role-customization', label: 'Role-Specific Customization', icon: 'users' },
    ],
  },
];
...
function NavigationItemView({ item, activeItem, onSelect, activeHubId }: { item: NavigationItem; activeItem: string; onSelect: (id: string) => void; activeHubId: string }) {
  ...
  if (activeHubId === 'superuser') {
    return <button type="button" onClick={() => { onSelect(item.id); router.push(`/superuser?view=${encodeURIComponent(item.id)}`, { scroll: false }); }} aria-current={active ? 'page' : undefined} className={classes}>{content}</button>;
  }
...
export function HubNavigation() {
  ...
  useEffect(() => {
    const sync = () => setActiveItem(currentView() || sections[0]?.items[0]?.id || '');
    sync();
