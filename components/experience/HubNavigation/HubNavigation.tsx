...
function getQueryKey(hubId:string){if(hubId==='athlete')return'age';if(hubId==='admin')return'role';if(hubId==='official')return'official_role';return'switcher';}
...
export function HubNavigation(){
 ...
 function handleSwitch(value:string){setSwitcherValue(value);if(typeof window==='undefined')return;const queryKey=getQueryKey(activeHubId);const query=new URLSearchParams(search);query.set(queryKey,value);if(activeHubId==='athlete'||activeHubId==='admin')query.delete('switcher');router.replace(`${pathname}?${query.toString()}`,{scroll:false});}
 ...
 function hrefFor(itemHref?:string){if(!itemHref)return'#';const origin=typeof window==='undefined'?'https://ls1sports.local':window.location.origin;const target=new URL(itemHref,origin);if(activeHubId==='athlete'&&switcherValue)target.searchParams.set('age',switcherValue);if(activeHubId==='admin'&&switcherValue)target.searchParams.set('role',switcherValue);return`${target.pathname}${target.search}`;}
 const showSwitcher=switcherConfig.displayStyle!=='none'&&switcherConfig.options.length>0;const switcherLabel=switcherConfig.type==='age'?'Demonstrate athlete experience':switcherConfig.type==='role'?'Select admin role':switcherConfig.type==='official_role'?'Select official role':'Select option';
