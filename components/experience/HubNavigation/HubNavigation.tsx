...
export function HubNavigation() {
  ...
  const [activeItem, setActiveItem] = useState(params.get('view') || sections[0]?.items[0]?.id || '');
  useEffect(() => { setActiveItem(params.get('view') || sections[0]?.items[0]?.id || ''); }, [params, activeHubId, sections]);
  const handleSelect = (id: string) => { setActiveItem(id); if (activeHubId === 'superuser' && !['/superuser'].includes(pathname)) router.push(`/superuser?view=${encodeURIComponent(id)}`); };
  return <nav className="flex h-full w-full flex-col bg-[#080909]">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-4">{sections.map(section => <div key={section.id} className="mb-5"><div className="mb-1.5 px-3 text-[8px] font-bold tracking-[0.2em] text-neutral-700">{section.label}</div><div className="space-y-0.5">{section.items.map(item => <NavigationItemView key={item.id} item={item} activeItem={activeItem} onSelect={handleSelect} />)}</div></div>)}</div>
