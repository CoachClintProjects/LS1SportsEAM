...
import { Activity, Archive, Award, Badge, BarChart3, Bell, Book, Bot, Boxes, Briefcase, Building, Calendar, CheckCircle, Clipboard, Command, Database, DollarSign, Eye, File, GitBranch, HeartPulse, History, Home, Key, Layers, Lightbulb, List, Lock, Map, Medal, Package, Plug, Receipt, Repeat, Rocket, Route, Search, Send, Server, Settings, Sparkles, Target, Timer, TrendingUp, Upload, User, UserPlus, Users, Wallet, Wrench, Workflow } from 'lucide-react';
...
export function HubNavigation() {
  ...
  const [ageBand, setAgeBand] = useState('5-8');
  const [activeItem, setActiveItem] = useState('');
  const sections = useMemo(() => {
    if (activeHubId === 'superuser') return [...getNavigation(activeHubId), ...superuserExtensions];
    if (activeHubId === 'athlete') return athleteNavigation(ageBand);
    return getNavigation(activeHubId);
  }, [activeHubId, ageBand]);
  ...
  function selectAge(nextAge: string) {
    ...
    const next = new URLSearchParams(window.location.search);
    next.set('age', nextAge);
    next.set('view', 'overview');
    window.dispatchEvent(new CustomEvent('ls1sports:athlete-age', { detail: nextAge }));
    window.dispatchEvent(new CustomEvent('ls1sports:navigation', { detail: 'overview' }));
