...
function buildSections(rows: DbNavRow[], hubId: string): NavigationSection[] {
  ...
  for (const row of rows) {
    ...
    if (!item) continue;
    if (row.parent_id && map.has(row.parent_id)) map.get(row.parent_id)?.children?.push(item);
    else roots.push(item);
  }

  roots.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
  for (const item of roots) item.children?.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
...
export async function getNavigation(hubId: string, switcherValue = ''): Promise<NavigationSection[]> {
  ...
  try {
    ...
    if (hubId === 'admin' && switcherValue) {
      ...
      if (role?.role_id) {
        const { data: permissions } = await supabase.from('hub_role_navigation').select('nav_id').eq('role_id', role.role_id).eq('can_view', true);
...
export async function getSwitcherConfig(hubId: string): Promise<SwitcherConfig> {
  if (hubId === 'athlete') return { type: 'age', displayStyle: 'radio', defaultOption: '5-8', options: [
    { id: '5-8', label: 'Foundation · Ages 5–8' }, { id: '9-11', label: 'Development · Ages 9–11' }, { id: '12-14', label: 'Growth · Ages 12–14' }, { id: '15-17', label: 'Performance · Ages 15–17' }, { id: '18+', label: 'Advanced · 18+' },
  ] };
  if (hubId === 'admin') return { type: 'role', displayStyle: 'radio', defaultOption: 'org_admin', options: [
    { id: 'org_admin', label: 'Organization Admin' }, { id: 'team_manager', label: 'Team Manager' }, { id: 'registrar', label: 'Registrar' }, { id: 'treasurer', label: 'Treasurer' }, { id: 'operations', label: 'Operations' }, { id: 'compliance', label: 'Compliance' }, { id: 'reporting', label: 'Reporting' },
  ] };
  if (hubId === 'official') return { type: 'official_role', displayStyle: 'radio', defaultOption: 'official', options: [
    { id: 'official', label: 'Official' }, { id: 'meet_referee', label: 'Meet Referee' }, { id: 'starter', label: 'Starter' }, { id: 'stroke_turn', label: 'Stroke & Turn' }, { id: 'judge', label: 'Judge' }, { id: 'meet_director', label: 'Meet Director' },
  ] };
