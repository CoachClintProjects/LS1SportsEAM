...
export function GlobalHeader() {
  ...
  useEffect(() => {
    function handleOutsideClick(event: MouseEvent) {
      if (hubMenuRef.current && !hubMenuRef.current.contains(event.target as Node)) setHubMenuOpen(false);
    }
        ...
        <div className="flex min-w-0 flex-1 items-center justify-center px-6">
          <div className="w-full max-w-[780px]">
            <div className="flex h-11 w-full items-center rounded-lg border border-neutral-800 bg-[#121414] px-4 transition-colors duration-150 focus-within:border-neutral-700">
              ...
              <input type="search" aria-label="Search LS1Sports" placeholder="Search LS1Sports..." className="min-w-0 flex-1 bg-transparent text-[13px] text-white outline-none placeholder:text-neutral-600" />
        ...
        <div className="flex shrink-0 items-center justify-end">
          ...
          <div ref={hubMenuRef} className="relative">
            <button
              ...
              className={`flex h-10 items-center gap-2 rounded-lg border px-3 transition-all duration-150 ${hubMenuOpen ? 'border-neutral-700 bg-neutral-900' : 'border-transparent hover:border-neutral-800 hover:bg-neutral-900'}`}
            >
                  ...
                  {hubs.map(hub => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left transition-colors duration-150 ${active ? 'bg-[#FA4616]/10 text-white' : 'text-neutral-400 hover:bg-neutral-900 hover:text-white'}`}
                      >
