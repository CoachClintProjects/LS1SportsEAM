...
export default function AthleteWorkspace() {
  ...
  function renderView(): React.ReactNode {
    ...
    if (view === 'documents') return <Panel><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-[#FA4616]"/><h3 className="font-black text-white">Document Vault</h3></div><p className="mt-2 text-sm text-neutral-500">Centralized, versioned and permission-governed. Sensitive files are not exposed by this sanitized demonstration projection.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(data?.documents??[]).length ? data!.documents.map((doc:Row)=><button key={String(doc.id)} onClick={()=>setDrawer({kind:'document',...doc})} className="rounded-xl border border-neutral-800 bg-[#0d1010] p-4 text-left hover:border-neutral-600"><div className="font-bold text-white">{String(doc.title)}</div><div className="mt-2 text-xs text-neutral-500">{String(doc.verificationStatus ?? 'unverified')} · {String(doc.classification ?? 'restricted')}</div></button>) : <Empty>No athlete-owned document records are connected to this live projection.</Empty>}</div></Panel>;
