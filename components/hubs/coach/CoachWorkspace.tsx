...
type Runtime = {
  ...
  attention: Array<{ id: string; category: string; horizon: string; title: string; summary: string | null; severity: string; status: string }>;
  proposals: Array<{ id: string; agent_key: string; proposal_type: string; autonomy_level: string; status: string }>;
...
const META: Record<View, { title: string; label: string; description: string }> = {
  command: { title: 'Coach Command Center', label: 'TODAY', description: 'Daily evidence, decisions and what needs coaching judgment next.' },
  squads: { title: 'Squads', label: 'TEAM', description: 'Active canonical team context and coaching population.' },
  rosters: { title: 'Rosters', label: 'ATHLETES', description: 'Real active athlete memberships for this team.' },
  attendance: { title: 'Attendance', label: 'EXECUTION', description: 'Session attendance evidence from canonical operations.' },
  training: { title: 'Training Plans', label: 'PLAN', description: 'Canonical plans. Empty means no operational plan exists yet.' },
  ...
  development: { title: 'Development', label: 'ATHLETE LOOP', description: 'Development grounded in real participation evidence.' },
  deck: { title: 'Deck Ledger', label: 'DECISIONS', description: 'Attention, AI proposals, judgment and accountability.' },
};

const VIEWS: View[] = ['squads', 'rosters', 'attendance', 'training', 'workouts', 'deployment', 'competition', 'performance', 'development', 'deck'];

function getView(pathname: string): View {
  const segment = pathname.split('/').filter(Boolean)[1] || '';
  return VIEWS.includes(segment as View) ? (segment as View) : 'command';
}
...
function Empty({ title, body }: { title: string; body: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6"><div className="text-sm font-semibold">{title}</div><p className="mt-2 max-w-3xl text-sm leading-6 text-white/50">{body}</p></div>;
}

function Stat({ label, value, detail }: { label: string; value: string | number; detail: string }) {
  return <div className="rounded-2xl border border-white/10 bg-[#0a0e0c] p-4"><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300/70">{label}</div><div className="mt-2 text-3xl font-semibold">{value}</div><div className="mt-1 text-xs text-white/40">{detail}</div></div>;
}

function isRuntime(value: Runtime | ErrorPayload | null): value is Runtime {
...
export default function CoachWorkspace() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as Runtime | ErrorPayload | null;
      if (!response.ok || !isRuntime(payload)) {
        throw new Error(payload && 'error' in payload ? payload.error : `Coach runtime returned ${response.status}.`);
      }
      setRuntime(payload);
    } catch (err) {
      setRuntime(null);
  ...
  const attendanceCounts = useMemo(() => {
    ...
    for (const record of runtime?.attendance.records || []) {
      const count = map.get(record.session_id) || { present: 0, absent: 0, other: 0 };
      const status = record.status.toLowerCase();
      if (['present', 'checked_in', 'attended'].includes(status)) count.present += 1;
      else if (['absent', 'no_show'].includes(status)) count.absent += 1;
      else count.other += 1;
      map.set(record.session_id, count);
    }
    return map;
  }, [runtime]);

  if (loading) return <div className="h-full bg-[#050807] p-6 text-sm text-white/55">Loading canonical Coach runtime…</div>;

  if (error) return <div className="h-full overflow-auto bg-[#050807] p-6 text-white"><div className="mx-auto max-w-5xl rounded-2xl border border-rose-400/20 bg-rose-400/5 p-6"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-rose-300">Coach runtime blocked</div><h1 className="mt-2 text-2xl font-semibold">Authorized Coach context could not be established.</h1><p className="mt-3 text-sm text-white/60">{error}</p><button type="button" onClick={() => void refresh()} className="mt-5 rounded-lg border border-white/15 px-4 py-2 text-sm hover:bg-white/5">Retry</button></div></div>;
  ...
  return <div className="h-full overflow-auto bg-[#050807] text-white"><div className="mx-auto max-w-[1500px] p-5 md:p-7">
    ...
    {!team ? <Empty title="No active team context" body="LS1 will not create a synthetic squad. Assign a real Coach team or use an authorized SuperUser team context." /> : <>
