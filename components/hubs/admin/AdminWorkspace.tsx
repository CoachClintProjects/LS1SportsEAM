...
export function AdminWorkspace(){const searchParams=useSearchParams();const router=useRouter();const view=searchParams.get('view')||'command-center';const ActiveComponent=useMemo(()=>viewRegistry[view]||CommandCenter,[view]);return <div className="relative min-h-full w-full"><button type="button" aria-label="Admin settings" title="Admin settings" onClick={()=>router.push('/admin?view=settings')} className="fixed bottom-6 right-6 z-[90] flex h-11 w-11 items-center justify-center rounded-full border border-neutral-700 bg-[#111313] text-neutral-300 shadow-xl hover:border-[#FA4616]/60 hover:text-[#FA4616]"><Settings className="h-5 w-5"/></button><ActiveComponent/></div>}
